...
class BuildingDetector(nn.Module):
    ...
    def forward_test(self, images, annotations=None):
        ...
        for b in range(remask_pred.size(0)):
            ...
            if not self.test_inria:
                ...
                for prop in props:
                    poly, juncs_sa, edges_sa, score, juncs_index = generate_polygon(
                        prop, mask_pred_per_im, juncs_pred, 0, self.test_inria
                    )
            ...
            extra_info.append(
                dict(
                    masks=dict(
                        afm_pred1=afm_pred[b, 0].cpu().numpy(),
                        afm_pred2=afm_pred[b, 1].cpu().numpy(),
                        mask_pred=mask_pred[b].cpu().numpy().squeeze(),
                        remask_pred=remask_pred[b].cpu().numpy().squeeze(),
                    ),
                    juncs=dict(
                        jloc_convex_pred=jloc_convex_pred[b].cpu().numpy().squeeze(),
                        jloc_concave_pred=jloc_concave_pred[b].cpu().numpy().squeeze(),
                        joff_pred1=joff_pred[b, 0].cpu().numpy(),
                        joff_pred2=joff_pred[b, 1].cpu().numpy(),
                    ),
                )
            )
    ...
    def forward_train(self, images, annotations=None):
        ...
        if targets is not None:
            loss_dict["loss_jloc"] += self.junc_loss(
                jloc_pred, targets["jloc"].squeeze(dim=1)
            )
            loss_dict["loss_joff"] += sigmoid_l1_loss(
                outputs[:, :], targets["joff"], -0.5, targets["jloc"]
            )
            loss_dict["loss_mask"] += F.cross_entropy(
                mask_pred, targets["mask"].squeeze(dim=1).long()
            )
...
def get_pretrained_model(cfg, dataset, device, pretrained=True):
    ...
    if pretrained:
        ...
        state_dict = torch.hub.load_state_dict_from_url(
            url, map_location=device, progress=True
        )
        state_dict = {
            k[7:]: v for k, v in state_dict["model"].items() if k[0:7] == "module."
        }
