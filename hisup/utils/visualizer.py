...
def viz_inria(image, polygons, output_dir, file_name, alpha=0.5, linewidth=12, markersize=45):
    ...
    for n, poly in enumerate(polygons):
        ...
        if poly.type == 'MultiPolygon':
            for p in poly:
                patch = PolygonPatch(
                    p.buffer(0), ec=poly_color, fc=poly_color, alpha=alpha, linewidth=linewidth)
                plt.gca().add_patch(patch)
                plt.gca().add_patch(Patches.Polygon(
                    p.exterior.coords[:-1], fill=False, ec=poly_color, linewidth=linewidth))
                juncs = np.array(p.exterior.coords[:-1])
                plt.plot(juncs[:, 0], juncs[:, 1], color=poly_color,
                         marker='.', markersize=markersize, linestyle='none')
                if len(p.interiors) != 0:
                    for inter in p.interiors:
                        plt.gca().add_patch(Patches.Polygon(
                            inter.coords[:-1], fill=False, ec=poly_color, linewidth=linewidth))
                        juncs = np.array(inter.coords[:-1])
                        plt.plot(juncs[:, 0], juncs[:, 1], color=poly_color,
                                 marker='.', markersize=markersize, linestyle='none')
        else:
            try:
                patch = PolygonPatch(poly.buffer(
                    0), ec=poly_color, fc=poly_color, alpha=alpha, linewidth=linewidth)
                plt.gca().add_patch(patch)
            except TypeError:
                plt.gca().add_patch(Patches.Polygon(
                    poly.exterior.coords[:-1], fill=True, ec=poly_color, fc=poly_color, linewidth=linewidth, alpha=alpha))
            plt.gca().add_patch(Patches.Polygon(
                poly.exterior.coords[:-1], fill=False, ec=poly_color, linewidth=linewidth))
    ...
    save_filename = os.path.join(
        output_dir, 'inria_viz', file_name[:-4] + '.svg')
...
def draw_predictions_with_mask_inria(img, junctions, polys_ids, save_dir, filename):
    ...
    for i, contours in enumerate(polys_ids):
        ...
        for h, idx in enumerate(contours):
            poly = junctions[idx]
            if h == 0:
                cv2.drawContours(instance_mask, [np.int0(
                    poly).reshape(-1, 1, 2)], -1, color=color, thickness=-1)
            else:
                cv2.drawContours(instance_mask, [np.int0(
                    poly).reshape(-1, 1, 2)], -1, color=0, thickness=-1)
